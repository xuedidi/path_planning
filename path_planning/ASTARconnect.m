...
drawnow;
 ...
 % if(max(ismember(setOpen,setOpen1(ii1)))||max(ismember(setOpen1,setOpen(ii))))
   %    break;
  %end
...
%        if ~isinf(cost(jj))    %如果此点的实际代价不为Inf,也就是没有遇到墙
      ...
      if ~max([setClosed; setOpen] == posinds(jj)) %如果此点不在OPEN表和CLOSE表中
           a= ~max(ismember(setOpen1,posinds(jj)));
          if(isempty(a)||~max(ismember(setOpen1,posinds(jj))))
 ...
 %        if ~isinf(cost(jj))    %如果此点的实际代价不为Inf,也就是没有遇到墙
      ...
      if ~max([setClosed1; setOpen1] == posinds1(jj))%如果此点不在OPEN表和CLOSE表中
          b=~max(ismember(setOpen,posinds1(jj)));
         if(isempty(b)||~max(ismember(setOpen,posinds1(jj))))
...
%    for jj=1:length(posinds1)      %对于扩展的四个方向的坐标
%     % if cost infinite, then it's a wall, so ignore
%     if(field(posinds1(jj))~=3 && field(posinds1(jj))~=5 && field(posinds1(jj))~=2)
%        field(posinds1(jj))=4; 
%       if ~isinf(cost1(jj))    %如果此点的实际代价不为Inf,也就是没有遇到墙
%       % if node is not in OPEN or CLOSED then insert into costchart and 
%       % movement pointers, and put node in OPEN
%       if ~max([setClosed; setOpen] == posinds1(jj)) %如果此点不在OPEN表和CLOSE表中
%         fieldpointers(posinds1(jj)) = movementdirections1(jj); %将此点的方向存在对应的fieldpointers中
%         costchart(posinds1(jj)) = cost1(jj); %将实际代价值存入对应的costchart中
%         setOpen = [setOpen; posinds1(jj)]; %将此点加入OPEN表中
%         setOpenCosts = [setOpenCosts; cost1(jj)];   %更新OPEN表实际代价
%         setOpenHeuristics = [setOpenHeuristics; heuristic1(jj)];    %更新OPEN表启发代价
%       % else node has already been seen, so check to see if we have
%       % found a better route to it.
%       elseif max(setOpen == posinds1(jj)) %如果此点在OPEN表中
%         I = find(setOpen == posinds1(jj));   %找到此点在OPEN表中的位置
%         % update if we have a better route
%         if setOpenCosts(I) > cost1(jj)  %如果在OPEN表中的此点实际代价比现在所得的大
%           costchart(setOpen(I)) = cost1(jj);    %将当前的代价存入costchart中，注意此点在costchart中的坐标与其自身坐标是一致的（setOpen(I)其实就是posinds(jj)），下同fieldpointers
%           setOpenCosts(I) = cost1(jj);      %更新OPEN表中的此点代价，注意此点在setOpenCosts中的坐标与在setOpen中是一致的，下同setOpenHeuristics
%           setOpenHeuristics(I) = heuristic1(jj);    %更新OPEN表中的此点启发代价(窃以为这个是没有变的)
%           fieldpointers(setOpen(I)) = movementdirections1(jj);   %更新此点的方向   
%         end
%       end
%      end
%     end
%    end
%   end
% if(max(ismember(setClosed,setOpen1(ii1)))||max(ismember(setClosed1,setOpen(ii))))
%        break;
% end
%if isempty(setOpen)
...
if(max(ismember(setOpen,setOpen1)))
   ...
   b=intersect(setOpen1,setOpen);
%    n = length(fieldpointers);  % length of the field
     if(max(ismember(posinds1,b)))
         posind = b;
       ...
       p1=flipud(p1); 
     posind5=b;
     [px1,py1] = ind2sub([10,10],posind5);
       p6 = [px1 py1];
       p7=posind5;
     xuedidi=find(posinds1==b);
     fieldpointers(b)=movementdirections(xuedidi);
      while ~strcmp(fieldpointers{posind5},'G')
            switch fieldpointers{posind5}
             case 'L' % move left  如果获得该点的来源点方向为左时
                py1 = py1 +1;
             case 'R' % move right
                py1 = py1 - 1;
